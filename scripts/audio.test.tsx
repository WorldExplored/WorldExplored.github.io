...
test('recorded coast starts only on a gesture, positions its layers, and rings the muted-aware bell independently',async t=>{
  const html=renderToStaticMarkup(<EnvironmentalAudioControl/>);assert.match(html,/aria-label="Ambience"/);assert.doesNotMatch(html,/Pause ambience/);assert.doesNotMatch(html,/src=|autoplay|<audio|<iframe/);
