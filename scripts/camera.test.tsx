...
import { CAMERA_LIMITS, cameraObstacles, clipCameraTravel, constrainCameraPose, focusPose, intersectTerrainRay, zoomTowardPoint } from '../src/components/world/cameraControls';
...
function assertSafe(position: Vector3, target: Vector3) {
  ...
  assert.ok(position.y >= Math.max(CAMERA_LIMITS.minY, terrainHeight(position.x, position.z) + 1.15) - 1e-6, 'The near plane clears terrain and water.');
...
test('wheel zoom preserves the terrain point under the cursor until a safety bound is reached', () => {
  ...
  assert.ok(Math.abs(hit.y - Math.max(0, terrainHeight(hit.x, hit.z))) < .001);
...
function pointer(surface: MockSurface, type: string, x: number, y: number, pointerId = 1, pointerType = 'mouse') {
  const event = new Event(type, { cancelable: true });
  Object.assign(event, { clientX: x, clientY: y, pageX: x, pageY: y, pointerId, pointerType, button: 0 });
