...
export function SectionContent({ id }: { id: SectionId }) {
  if (id === 'work') return <>
    <div className="contribution-collection">{publicContributions.map(item => <a className="contribution-card" key={item.number} href={item.url} target="_blank" rel="noopener noreferrer" aria-labelledby={`pr-${item.number}`}>
      ...
      <h3 id={`pr-${item.number}`}>{item.heading}</h3><h4>Problem</h4><p className="problem">{item.problem}</p><h4>Why it mattered</h4><p>{item.why}</p><h4>What I changed</h4><p>{item.contribution}</p><h4>Validation</h4><p>{item.validation}</p><h4>Status</h4><p>{item.currentStatus}</p>
