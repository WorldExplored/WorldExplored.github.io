...
import { combine, platform, roundedBox, strut, usePalette, useResources, type ModelProps } from './BuildingKit';

export function makeReceptionInterior() {
  const room = new InteriorBuilder();
  room.floor('contact-lobby-and-service-wood-floor',[floorEllipse(0,0,1.51),floorRectangle(1.69,-.25,1.08,1.99)],1.086,.014);
...
export function ReceptionTerminal(props: ModelProps) {
  ...
  const geometry=useResources(()=>{
    ...
    return {
      foundation:floorSlab('contact-foundation',[floorEllipse(0,0,1.71),floorRectangle(1.75,-.25,1.24,2.29)],.99,.19,'foundation'),
      floor:floorSlab('contact-room-floors',[floorEllipse(0,0,1.535),floorRectangle(1.695,-.25,1.09,2.015)],1.069,.079),
      ...
      serviceWing:combine([
        roundedBox(1.45,.16,2.33,.08).translate(1.73,3.06,-.25),
  ...
  return <group dispose={null}>
    ...
    <mesh name="contact-room-floors" geometry={geometry.floor} material={material.porcelain} receiveShadow />
