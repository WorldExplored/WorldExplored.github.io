...
import { distanceToSegment, landDistance, seededRandom, terrainMeshHeight, terrainSlope, ISLANDS, islandAt, islandContour, type LandscapePlan } from './terrain';
...
export function createShoreDetails(plan:LandscapePlan) {
  ...
  // Skip interior meadow patches covered by the enlarged city shoreline.
  // Sample only exposed coast owners, so an inland patch cannot exhaust a batch.
  const coastPatches=ISLANDS.map(island=>{
    const patches:Array<{x:number;z:number;radius:number}>=[];
    for(let sample=0;sample<72&&patches.length<18;sample++){
      const a=(sample+random())*Math.PI*2/18,contour=islandContour(island,a),inset=.6+random()*1.8;
      const x=island.x+Math.cos(a)*(island.rx*contour-inset),z=island.z+Math.sin(a)*(island.rz*contour-inset);
      const shore=islandAt(x,z);
      if(shore.island.id!==island.id||shore.distance<.35||shore.distance>2.8)continue;
      patches.push({x,z,radius:.3+random()*.85});
    }
    return patches;
  }).filter(patches=>patches.length);
