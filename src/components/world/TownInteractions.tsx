...
import { COASTAL_BELL_EVENT, emitTechnologySound } from './coastalAudio';
...
function TownHitTarget({ site, controls, resources }: { site: TownInteractionSite; controls: TownInteractionState; resources: ReturnType<typeof createTargetResources> }) {
  ...
  const activate = () => { controls.activate(site.id); if (site.id !== 'buoy' && site.id !== 'fountain') emitTechnologySound('servo', site.position); if (site.id === 'buoy' && typeof window !== 'undefined') window.dispatchEvent(new Event(COASTAL_BELL_EVENT)); };
