...
import { seededRandom, landDistance } from './terrain';
...
export interface MarineResident {kind:ResidentKind;index:number;size:number;radius:number;position:Vector3;home:Vector3;target:Vector3;heading:number;pitch:number;time:number;moving:boolean;nextMove:number;jet:number;distance:number;random:()=>number}
export const RESIDENT_COUNTS={'crawling-octopus':4,'sea-snake':3,'squid':5} as const;
...
export function residentHeight(kind:ResidentKind,x:number,z:number,time:number,index:number){
  ...
  return kind==='squid'?Math.min(-1.35,floor+1.4+Math.sin(time*.31+index)*.20):floor+(kind==='sea-snake'?.18:.055);
}
...
export function createMarineResidentsState(){
  ...
  for(const kind of Object.keys(RESIDENT_COUNTS) as ResidentKind[])for(let index=0;index<RESIDENT_COUNTS[kind];index++){
    ...
    const radius=kind==='crawling-octopus'?size*.91:kind==='sea-snake'?size*.9:size*.75;
    ...
    states.push({kind,index,size,radius,position,home:position.clone(),target:position.clone(),heading:random()*Math.PI*2,pitch:0,time:0,moving:false,nextMove:index===0?1:3+random()*7,jet:0,distance:0,random});
  }
  return states;
}
function chooseResidentTarget(state:MarineResident){
  for(let attempt=0;attempt<90;attempt++){
    const a=state.random()*Math.PI*2,length=state.kind==='squid'?2+state.random()*4:.7+state.random()*2;
    ...
    let clear=true;
    for(let step=0;step<=20;step++)if(!residentPositionClear(state.position.x+(x-state.position.x)*step/20,state.position.z+(z-state.position.z)*step/20,state.radius)){clear=false;break;}
    if(!clear)continue;
...
export function stepMarineResidents(states:MarineResident[],delta:number,paused=false){
  ...
  for(const state of states){
    state.time+=dt;
    if(state.kind==='squid')state.position.y=residentHeight(state.kind,state.position.x,state.position.z,state.time,state.index);
    if(!state.moving&&state.time>=state.nextMove)chooseResidentTarget(state);
    state.jet=state.kind==='squid'?Math.max(0,Math.sin(state.time*2.2+state.index*1.7))**5:0;
    if(state.moving){
      ...
      const speed=state.kind==='squid'?.16+state.jet*1.45:state.kind==='sea-snake'?.25:.10+.08*Math.sin(state.time*2)**2;
      ...
      const y=residentHeight(state.kind,x,z,state.time,state.index);
      ...
      if(distance<.05){state.moving=false;state.nextMove=state.time+(state.kind==='squid'?1+state.random()*3:state.kind==='sea-snake'?8+state.random()*15:9+state.random()*18);}
    }
    const dx=Math.cos(state.heading)*.25,dz=-Math.sin(state.heading)*.25;
    state.pitch=state.kind==='squid'?.04*Math.sin(state.time*.6):Math.atan((marineFloorHeight(state.position.x+dx,state.position.z+dz)-marineFloorHeight(state.position.x-dx,state.position.z-dz))/.5);
  }
}
